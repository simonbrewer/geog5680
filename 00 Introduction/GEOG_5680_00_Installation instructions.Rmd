...
IMPORTANT NOTE: A new version of R (4.0.0) was released just before the start of this class. The code in the labs will be updated to work on this version. If you have an older version (3.x), these should still work, but I will try to note where any changes are necessary. If you have the time, I would recommend updating to R 4.0.0.

## Installing R

R is available through the web page of [The Comprehensive R Archive Network]. At the top of this page, you will find three three links for downloading R depending on your operating system: Windows, Mac, or Linux.
...
To install R on a Mac, click the “Download R for Mac” link, then download the package link (`R-4.0.0.pkg`). Once this has downloaded, click on the file to run the installer. There are some options during the installation, but I'd recommend using the defaults for these. If your computer requires a password before installing new progams, you’ll need it here.

### Linux

R comes preinstalled on many Linux systems, but is often an older version, and you'll likely want to update this. Click on the “Download R for Linux.”, which will take you to a new page giving options for some of the more widely used Linux distros: Debian, Redhat, SUSE, and Ubuntu. Clicking on any of these will take you to a new page with detailed installation instructions for that system. Note that you can also download the source code from the first page and compile it yourself. This is realtively straightforward on most Unix-type operating systems. 
...
In any of these windows, you can now type R commands. We'll be using RStudio for this class (see below), but it is worth at least being familar with the basic R interface in case you are ever working on a different computer. Just to test this, type the following in the console (you should see the cursor next to the `>` symbol)
