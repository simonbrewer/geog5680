...
**This is one of the longest labs in this class, and contains some boring, but very useful information about working with data in R. I would suggest working through it in 2 or even 3 sessions. If you do this, remember to save you code in a script so you can continue working on it, and remember to re-set your working directory if you close and re-open RStudio. **
...
The file *BirdFlu.csv* contains the annual number of confirmed cases and deaths of human Avian Influenza A/(H5N1) for several countries as reported to the World Health Organization (WHO). The data were taken from the WHO website (www.who.int/en/) and reproduced for educational purposes. 

Read the data into R, and use the functions `names()` and `str()` in R to view the data and get a overview of its structure. Make a new vector containing the number of bird flu cases by country in 2003 and then a second new vector containing the number of deaths by country for the same year. 

Use the `sort()` function to find the country has had the most deaths from bird flu in 2005
