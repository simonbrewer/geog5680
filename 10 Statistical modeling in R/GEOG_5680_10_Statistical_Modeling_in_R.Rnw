...
hist(Icecream$cons, xlab='Consumption pint/head', breaks=seq(0.1,0.6,0.05), main='Ice Cream Consumption', freq=FALSE)
xx = seq(0.1,0.6,by=0.01)
dd = dnorm(xx, mean(Icecream$cons), sd=sd(Icecream$cons))
lines(xx, dd, col=2, lwd=2)
@
\end{column}
\end{columns}
\end{frame}
...
plot(cons ~ temp, data=Icecream, 
     xlab='Temp degF', ylab='Consumption pint/head', 
     pch=16, main='Ice Cream Consumption')
@
\end{column}
\end{columns}
\end{frame}
...
Icecream<-read.csv("icecream.csv")
fit = lm(cons ~ temp, data=Icecream)
plot(cons ~ temp, data=Icecream, xlab='Temp degF', ylab='Consumption pint/head', pch=16, main='Ice Cream Consumption')
abline(fit, col=2, lwd=3)
yp <- predict(fit)
arrows(Icecream$temp,Icecream$cons,Icecream$temp,yp,length=0,angle=90)
@
	\end{column}
\end{columns}
\end{frame}
...
@
<<echo=FALSE>>=
meany <- rep(mean(regrex$y), length(regrex$y))
predy <- predict(ex1.lm)
plot(regrex$y ~ regrex$x, pch=16, col='red', xlab='x',ylab='y',main='Total Sum of Squares')
points(regrex$x, regrex$y, pch=1)
points(regrex$x, meany, pch=16, col='yellow')
points(regrex$x, meany, pch=1)
abline(h=mean(regrex$y), col='yellow')
arrows(regrex$x, meany, regrex$x, regrex$y, length=0)
@
	\end{column}
	\begin{column}{0.33\textwidth}
		Residual SS
<<echo=FALSE>>=
plot(regrex$y ~ regrex$x, pch=16, col='red', xlab='x',ylab='y',main='Error Sum of Squares')
points(regrex$x, regrex$y, pch=1)
points(regrex$x, predy, pch=16, col='green')
points(regrex$x, predy, pch=1)
abline(ex1.lm, col='green')
arrows(regrex$x, predy, regrex$x, regrex$y, length=0)
@
\end{column}
	\begin{column}{0.33\textwidth}
		Model SS
<<echo=FALSE>>=
plot(regrex$y ~ regrex$x, pch=16, col='red', xlab='x',ylab='y',main='Regression Sum of Squares')
points(regrex$x, regrex$y, pch=1)
points(regrex$x, meany, pch=16, col='yellow')
points(regrex$x, meany, pch=1)
abline(h=mean(regrex$y), col='yellow')
...
points(regrex$x, predy, pch=1)
abline(ex1.lm, col='green')
arrows(regrex$x, predy, regrex$x, meany, length=0)
@
	\end{column}
\end{columns}
\end{frame}
...
% % \begin{itemize}
% %   \item Intercept is \Sexpr{round(coef(fit1)[1],4)} --- for an actor of 0cm height
...
% % @
% % \begin{itemize}
% %   \item Intercept is now \Sexpr{round(coef(fit1)[1],2)} --- for an actor of average height
% % \end{itemize}
% % \end{frame}
% % 
% %--- Slide ----------------%
...
% @
% \begin{itemize}
%   \item Intercept is now \Sexpr{round(coef(fit.cen)[1],4)}: weight of an actor of \emph{average} height
% \end{itemize}
% \end{frame}
% 
% %--- Slide ----------------%
