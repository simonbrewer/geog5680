---
title: "GEOG 5680 6680 14 R and Shiny"
author: | 
  | Simon Brewer
...
Note that these are included in the function `fluidPage()`, which allows the interface to adapt to the size of any browser window. 
...
You should be getting familiar with most of this code. Some notes:
...
In the *ui.R* script add the following code:
...
- A Spatial*DataFrame object (here, taken from a shapefile)
- The variable in the object to plot
- The number of breaks to use in the color palette
- The palette to use. This is one of the RColorBrewer palettes
...
In *helpers.R* modify the function to include a parameter for log-transformation:
```{r eval=FALSE}
...
[**Note that there is currently a problem with the latest version of R (4.0.0) in connecting to this service. This should be fixed fairly quickly**]
...
The app is designed to let you select a country and a variable to plot (e.g. number of deaths or cases), and it will then render this in the main window. While it should work out of the box, there are some problems with. For the exercise, you will need to fix this, then submit your code as well as a Word document with a screenshot of the working corrected app, and the answer to the first two questions below:
