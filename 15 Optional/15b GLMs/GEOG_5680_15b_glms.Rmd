---
title: "GEOG 5680 6680 15c Generalized linear models in R"
author: | 
  | Simon Brewer
...
The expected value of $y$ is again related the linear predictor, but by using a *link* function. Basically this is a way of transforming $y$ so that the relationship betweem the *transformed* $y$ and $X$ is linear:
...
We can also use the `predict()` function to show what the model looks like, by predicting probabilities across a range of DVRT scores. Note that once we have made the predcition, we add the mean back on to the centered DVRT values to plot on the original scale. THe plot we get shows the classic sigmoid shape of a logistic or binomial regression:
